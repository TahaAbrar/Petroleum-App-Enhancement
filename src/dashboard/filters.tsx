...
function DatePill({
  ...
  return (
    <div ref={rootRef} className={`relative ${fullWidth ? 'flex-1' : ''} ${open ? 'z-[80]' : 'z-0'}`}>
      ...
      {open ? (
        <div
          ...
          className={`absolute top-[calc(100%+6px)] z-[80] w-[min(17.5rem,calc(100vw-1.5rem))] rounded-2xl border border-line bg-white p-3 shadow-[0_12px_32px_rgba(26,29,33,0.14)] ${
...
export function MenuFilter({
  ...
  return (
    <div ref={rootRef} className={`${boxClass(variant, fullWidth)} ${open ? 'z-[80]' : ''}`}>
      <button
        type="button"
        className="absolute inset-0 z-[1] cursor-pointer rounded-[inherit] border-0 bg-transparent"
...
export function SearchableCustomerFilter({
  ...
  return (
    <div ref={rootRef} className={`${variants.boxFull} ${open ? 'z-[80]' : ''}`}>
      <button
        type="button"
        className="absolute inset-0 z-[1] cursor-pointer rounded-[inherit] border-0 bg-transparent"
        aria-label={ariaLabel}
